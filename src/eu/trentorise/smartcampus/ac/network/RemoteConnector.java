...
public class RemoteConnector {
	...
	/** address of the create email endpoint */
	private static final String PATH_CREATE_EMAIL = "/getToken/email";
	...
	public static UserData createEmailUser(String service, String email) {
		String requestString = service + PATH_CREATE_EMAIL;
		Log.i(TAG, "create email user: " + email);
		if (email != null) {
			requestString += "?email=" + email;
		} else {
			Log.i(TAG, "Failed creating email user: email address requred");
			return null;
		}
		final HttpGet request = new HttpGet(requestString);
		// HTTP parameters stores header etc.
		...
		try {
			...
			if ((accessCode != null) && (accessCode.length() > 0)) {
				Log.v(TAG, "Successfully created: " + email);
				return validateAccessCode(service, accessCode);
			} else {
				Log.e(TAG, "Error creating email user: http code " + resp.getStatusLine());
				return null;
			}
		} catch (Exception e) {
			Log.e(TAG, "Error creating email user: " + e.getMessage());
			return null;
		}
	}
	...
	public static List<Attribute> getUserAttributes(String host, String authority, String key, String token)
			throws SecurityException, AcServiceException {
		String json = null;
	...
	public static boolean canReadResource(String host, String token, String resourceId) throws SecurityException,
			AcServiceException {
